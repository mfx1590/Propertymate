...
### Build status (Phase 2, §10.2 — continues into Phase 3 from step 23)
...
| 21 | §13.2 identity-level ban durability | ✅ **Done (as far as a hash honestly goes)** | 2026-09-03 | §13.2 claims a ban is durable "because identities are verified", but only the unique `users.phone` enforced that — the same person could re-register on a new number with the same passport. New `banned_identities` records the SHA-256 of a banned account's **identity** documents (`government_id`, `owner_id`, `selfie_with_id`, `signatory_id` — a shared utility bill says nothing about who holds an account), written from **both** ban paths so durability does not depend on which admin screen was used. Surfaced as a distinct, louder signal than the existing §4 file-reuse alert (reuse can be innocent; this cannot), and **approval is refused outright** — by that point the flag has already been shown once, and approving is the exact action the ban exists to prevent. **Reinstating clears the list**, or a reversed ban would keep locking someone out through a door nobody remembers closing. **What this honestly is not:** hash matching catches the same *file* being re-uploaded, not the same person — a re-photographed passport hashes differently. Real identity matching needs OCR of the document number. The durable part is therefore the human verification step, and this makes sure the reviewing admin is told; the code says so rather than implying more. **Hazard found while testing:** every e2e fixture uploads the same 1×1 JPEG, so all identity documents share one hash — a banned identity left behind by a failed run would refuse profile approvals in every later suite. The suite now reinstates in `finally`, verified by injecting a failure after the ban and confirming the list came back empty. Suite at 49 checks. |
| 22 | §6.1 polygon-draw filter + POI layers | ✅ **Done** | 2026-09-03 | The last two §6.1 items, and the end of the discovery surface. **The whole problem is that a box is not a shape.** Meilisearch 1.11 has `_geoBoundingBox` but no polygon filter (`_geoPolygon` lands in a later version, and upgrading the engine to get one filter was not worth it), so the box is the coarse pass and a ray-casting `pointInPolygon` narrows it exactly, in one place — `SearchService.execute` — which every caller now goes through. That consolidation is the point: the alternative was honouring an area on the search page and quietly ignoring it in the nightly alert sweep, where nobody would ever see it. The suite asserts the difference directly by parking a listing in the corner of a triangle's bounding box: it is returned for the box and refused for the triangle, and the same pair is run again through a saved search so a bbox-only sweep would fail there too. Paging and the count are computed **after** the exact test rather than before, or the page numbers would describe a set the map does not show; a box holding more than one 1,000-document scan reports `scanLimited` and the UI says "showing the first 1,000" instead of a confident wrong total. The area is a query parameter (`lat,lng;…`), not a POST body, so a drawn search stays shareable and bookmarkable like every other filter — and `?polygon=` on a cold URL opens the page in map view already filtered. **Dropping the area is a relaxation** (`RELAXABLE` gained `polygon`), because an over-tight shape is the easiest filter to draw by accident and the hardest to see; conversely **nearby-region suggestions are suppressed while an area is drawn**, since their counts come from the facet distribution inside the box and would overstate, and "try Nicosia" is incoherent advice to someone who has just traced a coastline. **POI layers** ship as a static catalogue of 25 landmarks — 9 universities, 10 beaches, 6 hospitals — served as GeoJSON from `GET /pois` (`?category=` narrows) with `GET /pois/near` answering "how far is the beach" on the listing page, server-rendered so a crawler gets it too. Held in **TypeScript, not the `.geo.json` the plan names**: `nest-cli.json` declares no `assets`, so a JSON file under `src/` never reaches `dist/` and the endpoint would have worked in watch mode and 500ed in production. What leaves the API is still GeoJSON. Each POI carries its Turkish name alongside the English one, because that is what the road sign says. Distances are labelled approximate and straight-line at every point they appear — in this terrain a straight line is not a drive time. **Drawing is plain Leaflet**, not `leaflet-draw`: click to add a vertex, click the first to close, and the map deliberately **stops refitting its bounds while an area is drawn or in force**, since the results change because of the shape on screen and refitting would drag the map out from under the hand drawing it. Vertex handles `stopPropagation` or closing the shape would add a fifth point on top of the first. Under three vertices no query is sent at all — a half-drawn shape must not narrow anything. Verified in the browser end to end: drawing cut 128 listings to 40, the chip and the filtered count survived a switch to list view (an invisible filter is how a user concludes the site is broken), clearing restored 128, and the Farsi RTL page rendered the whole toolbar and a URL-supplied polygon correctly. 44-check e2e suite ([map-search-flows.mjs](apps/api/test/e2e/map-search-flows.mjs), `test:e2e:map`, wired into CI). 1103-key parity across four locales. **Not built:** the mobile app keeps the plain map — tracing a polygon with a thumb is a different interaction, not a port of this one. |
| 23 | Phase 3 — lawyer marketplace (§10.2, §2.2 service-provider activation) | ✅ **Done** | 2026-09-03 | The first lateral role switched on, and the first real test of the §2.2 claim that adding one would be "config + one module, zero changes to core". **It very nearly held.** Core took three additions and no rewrites: a `lawyer` row in `ROLES`, a permission list, and three `verification_requirements` rows — §2.2 predicted the bar licence would be "one seed row" and it is three, because a licence alone does not prove the holder is the person uploading it. Registration, the profile endpoint with its field whitelist, the verification queue, the badge and the dashboard shell all took a lawyer without modification; the e2e drives every one of them through the **generic** surface on purpose, since a check that passed only through a lawyer-specific endpoint would prove the opposite of what it looks like. The two genuine core edits were both extensions rather than special cases: the profile editor learned `list` and `select` field kinds (a lawyer's regions and languages are what the directory filters on, so they could not be text-only), and `DealsService` gained a public `currentStageDef` because the injection points live on the stage. **The headline is that `injectableServiceTypes` is finally read.** It has been declared on five stages across all three pipelines since Phase 1 and *nothing has ever consulted it* — the saved-searches gap again. Now a quote request is authorised by the stage: refused at `reservation`, allowed one stage later at `legal_check`, with nothing changed but the deal's position in a pipeline defined in data. The suite asserts that pair directly, and the refusal names the stages that would work rather than just saying no. A rental correctly admits no lawyer at all, because its template declares none — config speaking, not a rule written in the module. **The engagement belongs to the requester, not the deal.** Buyer and seller each engage their own lawyer, because one lawyer acting for both sides of a conveyance is a conflict of interest; every read is scoped by who is asking, so the seller sees nothing of what the buyer is paying, and the suite asserts that boundary from both directions. Accepting creates the `deal_parties` row, adds the lawyer to the deal-room conversation, and **auto-closes the requester's other open requests** — a lawyer who quoted and then hears nothing has no way to learn the work went elsewhere. Those closures are captured *before* the transaction, or reading them back by status afterwards would re-notify every request the client had withdrawn weeks earlier. **Deliberately not built:** lawyers are not rateable and carry no ranking score. The §6.5 formula is built from listing-side inputs — deal count, response time on property threads — that say nothing about legal work, so the directory orders on accepted engagements *on this platform* and claims nothing more. **Two defects caught in the browser rather than by the suite:** the lawyer's inbox rendered "this property" for every off-plan deal, because the title lookup only knew about `property` and an off-plan deal hangs off a project unit — which is most of what a TRNC lawyer would ever see; and the quote picker printed raw `kyrenia` and `en` at the user, the same raw-key defect step 16 caught on region titles. The suite's title check had passed on a truthy string, and now asserts the actual unit name. **Also fixed:** the public payload was checked to carry no bar number, phone or documents — verification is the platform's job and its result is the badge, not the paperwork. Six `legal.*` notification templates in four locales, with every placeholder locale-agnostic (a name, a title, a number, a currency code) after step 20's lesson about English labels baked into payloads rendered per-recipient later. Web: public `/lawyers` directory + profile (in the sitemap, and **verified to build with the API unreachable**, which is what CI actually does), the deal-room panel, and a lawyer dashboard whose menu is deliberately two entries — the listings/leads/analytics every other pro role carries would all be empty screens. 57-check e2e suite ([legal-marketplace-flows.mjs](apps/api/test/e2e/legal-marketplace-flows.mjs), `test:e2e:legal`, wired into CI). 1184-key parity across four locales. |
...
**Phase 3 has started.** The lawyer marketplace (step 23) is the first §10.2 Phase-3 item and the first
lateral role activated — `service_providers` and the stage-level injection points are no longer seams.
Still unbuilt in Phase 3: the **payments module** (see the escrow scoping brief and the Change Log row
below — the deposit half is blocked on a written legal opinion, the subscription half only on a provider),
**market-insights pages + valuation v1**, the full dispute workflow, admin data export, and chat translation.

**Phase 2 (§10.2) is complete** as of 2026-08-21 — the Expo app was the last item. **§6.1 is now fully built**: alerts in step 14, compare view and multi-currency in step 15, region landing pages and the sitemap in step 16, calculators and payment schedules in step 17, and the polygon-draw filter and POI layers in step 22. Sorting by price/m² was already wired into Meilisearch. The only §6.1 line left unclaimed is the **monthly market-insights page**, which the spec itself defers to Phase 3 data. **Still open,
carried forward:** the mobile app covers the customer core only, so the agent-facing screens (my
listings, lead inbox) and the listing wizard remain web-only by choice, and its map stays a plain
pin view — tracing a polygon with a thumb is a different interaction, not a port of the web one. **Farsi contract rendering was dropped** on 2026-09-03 (Change Log) rather than
carried any further: step 19 covers EN/TR/RU, and a Farsi reader gets the English document under a
banner naming the gap. (§6.2 agent-mandate e-sign landed in step 18; purchase and off-plan sale
agreements in step 20, so every pipeline now generates its own document.) §13.2 **review moderation** was built in step 13 and its ID-level ban
durability in step 21 — as far as a file hash can go, which is same-file detection rather than same-person.
Matching a re-photographed document needs OCR of the ID number and is still open. Notification channels are code-complete but
...
| 2026-09-03 | **Farsi contract rendering is dropped, not deferred.** Mehdi: "farsi contract is not important." It had been the last Phase-2 carried-forward item, blocked on an Arabic shaping pass (fontkit `layout` feeding positioned glyphs, or HTML-to-PDF) — a font alone was measured as insufficient. It is now a settled scope decision rather than a queue entry, and no shaping work is planned. **What a Farsi reader gets is unchanged and already correct:** `contractLocale()` falls back to English and the PDF carries the `NOT_IN_YOUR_LANGUAGE` banner naming the gap and telling them to obtain a certified translation before signing. The banner is written in English on purpose — a notice in a script we cannot draw would be stripped to nothing. Farsi remains fully supported everywhere else in the product; this is the contract PDF only. | Mehdi | §0 (this row), §10.2 step 19 |
| 2026-09-03 | **Payments scoped before it is built; escrow separated from the rest.** Asked to start the payments conversation early because of its lead time, a scoping pass separated three flows that the plan's single word "payments" had been hiding: subscriptions + featured slots (platform revenue, blocked only on finding an acquirer that will onboard a TRNC entity), success fees (blocked on tax and invoicing treatment), and **escrow deposits — where the money is never the platform's**. If a buyer's deposit ever lands in an account the platform controls, the platform is holding client money, and that, not the software, is the regulated act; the design question is therefore how to stay out of the custody chain while still reflecting its state in the deal room. **Two concrete gaps found in the code:** `plans` has no price, currency or interval column, so a plan cannot express what it costs; and the `payments` module §2.3 lists as "stub until Phase 3" does not exist in the source tree at all. Build order recorded: price a plan → payments module with a manual provider (makes today's admin grant auditable, zero legal exposure) → card collection → success-fee invoicing → escrow, the last gated on a written legal opinion from a TRNC advocate. No code written; the legal and provider questions are Mehdi's to put to a lawyer and an acquirer. | Mehdi | §0 (this row), §9, §13.3, §10.2 |
...
- **Service Provider abstraction (build the interface now, even though empty at launch):**
  ...
  *(**Activated 2026-09-03** for `lawyer` — step 23. The table and the stage-level injection points are
  no longer seams; the remaining service types still are.)*
