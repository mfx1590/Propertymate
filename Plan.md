...
token endpoints are built and waiting for it); recommendation v1 (`property_view_events` now
exists — the co-visitation job does not); rental contract PDF generation + typed e-sign (also
blocks the §6.2 mandate e-sign); referral system. Notification channels are code-complete but
...
Three dashboard nav entries still fall through to the Coming Soon catch-all: `/dashboard/leads`
(§6.2 lead inbox), `/dashboard/admin/users`, `/dashboard/admin/disputes` — and §13.2 review
moderation (report → queue → warnings → ban) is unbuilt, with the `disputes` table still unused.
