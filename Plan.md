...
**Still open in Phase 2 (§10.2):** Expo mobile app (`apps/mobile` does not exist yet); push +
WhatsApp + email notification senders (the channel seam exists, only `in_app` is live);
recommendation v1 (`property_view_events` now exists — the co-visitation job does not); rental
contract PDF generation + typed e-sign (also blocks the §6.2 mandate e-sign); referral system.
