...
listings, lead inbox) and the listing wizard remain web-only by choice; purchase/off-plan sale-agreement templates (rental and mandate are the only
templates today), and **Farsi contract rendering**, which needs an Arabic shaping pass rather than a font
(step 19 covers EN/TR/RU). (§6.2 agent-mandate e-sign landed in step 18.) §13.2 **review moderation** was built in step 13; the one piece of it still
