...
**Still open in Phase 2 (§10.2):** Expo mobile app (`apps/mobile` does not exist yet — the push
token endpoints are built and waiting for it — it is the only §10.2 item left); the §6.2 **agent-mandate** e-sign
(the `agent_mandate` contract kind and the signature machinery exist — only the mandate template
and the flow that triggers it are outstanding); non-English contract PDFs (needs an embedded
Unicode font) and purchase/off-plan sale-agreement templates. §13.2 **review moderation** (report a review → admin queue → warnings → ban) is still
unbuilt — the dispute centre added in step 9 covers deal disputes, not profile reviews, though it
reuses the same admin patterns. Notification channels are code-complete but
