...
**Phase 2 (§10.2) is complete** as of 2026-08-21 — the Expo app was the last item. **§6.1 is still only partly built** beyond the alerts added in step 14: compare view (up to 4), prices converted to the viewer's currency (the `FxRate` table exists and nothing reads it), mortgage/installment calculators with developer payment-plan schedules, region landing pages, polygon-draw filter and POI layers are all specified and unbuilt. Sorting by price/m² is already wired into Meilisearch. **Still open,
