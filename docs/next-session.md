...
A SECOND WINDOW is building the landing page on branch `feat/landing` from
docs/landing-concept.md. It owns apps/web/src/app/[locale]/page.tsx,
apps/web/src/components/landing/**, the `home` i18n namespace and the
landing media. Do not touch those paths. Pull before every push.
...
```
Build the Propertymate landing page (C:\Users\mehdi\Desktop\Propertymate).
...
BEFORE ANYTHING: Docker Desktop is usually not running after a reboot — start
it, then `docker compose up -d --wait`. MinIO is on 9200/9201 here. The web
dev server is `npm run dev -w apps/web` (or preview_start "web").
